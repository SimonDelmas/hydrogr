...
from hydrogr.models import gr4h
...
class ModelGr4h(ModelGrInterface):
    ...
    frequency = ['H']
    n_param = 4
    n_states = 7 + 3 * 20 * 24  # Cemagref Fortran implementation reserve 7 float before unit hydrographs vectors. Seems to be the case for all model.
    output_name_list = ["PotEvap", "Precip", "Prod", "AE", "Perc", "PR", "Q9", "Q1", "Rout", "Exch", "AExch", "QR", "QD", "Qsim"]
    ...
    def _get_state_start(self):
        ...
        state_start[7: 7 + 20 * 24] = self.uh1
        state_start[7 + 20 * 24: 7 + 3 * 20 * 24] = self.uh2
        return state_start
